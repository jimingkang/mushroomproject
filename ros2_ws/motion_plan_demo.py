...
class MotionPlanner(Node):
    ...
    def add_cylinder_once(self):
        ...
        cylinder.dimensions = [0.8, 0.02]  # 高度=0.4m, 半径=0.08m
    ...
    def plan_and_print_once(self):
        ...
        #state = self.moveit2.get_current_joint_positions()
        #for i in range(len(state.joint_state.velocity)):
        #    state.joint_state.velocity[i] = 0.0
        #self.moveit2.set_start_state(state)

        self.get_logger().info("📡 开始规划路径（考虑障碍物）...")

        traj = self.moveit2.move_to_pose(
            position=[0.1,0.5, 0.01],   # 目标点在障碍物后方
            quat_xyzw=[0.0, 0.0, 0.0, 1.0]
        )
        #traj=self.moveit2.move_to_configuration([0.1, 0.5, -0.3, 0.0])

        if traj is None:
            self.get_logger().error("❌ 规划失败，可能无法绕过障碍物")
            return

        self.get_logger().error(traj)
