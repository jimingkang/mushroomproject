...
def draw_detections(image, boxes, scores, class_ids, mask_alpha=0.3):
    ...
    for box, score, class_id in zip(boxes, scores, class_ids):
        color = colors[class_id%80]

        x1, y1, x2, y2 = box.astype(int)
        if score > 0.5:
            detections.append([x1, y1, x2, y2, score])
        tracker.update(det_img, detections)
        for track in tracker.tracks:
            bbox = track.bbox
            x1, y1, x2, y2 = bbox
            track_id = track.track_id
            cv2.rectangle(det_img, (int(x1), int(y1)), (int(x2), int(y2)), (colors[track_id % len(colors)]), 3)
            print(track_id)
        # Draw rectangle
        #cv2.rectangle(det_img, (x1, y1), (x2, y2), color, 2)

        # Draw fill rectangle in mask image
        #cv2.rectangle(mask_img, (x1, y1), (x2, y2), colors[track_id % len(colors)], -1)

        label = class_names[class_id%80]
        ...
        #              (x1 + tw, y1 - th), colors[track_id % len(colors)], -1)
